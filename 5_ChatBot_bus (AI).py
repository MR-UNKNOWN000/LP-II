...
while True:
    ...
    '''✅ Title:
Rule-Based RedBus Chatbot Using Python

✅ Algorithm:
Convert user input to lowercase and strip any whitespace.

Use regex pattern matching to identify the intent of the message.

If a matching pattern is found, return the corresponding predefined response.

If no pattern matches, return a fallback message.

✅ Short Theory:
This is a rule-based chatbot for RedBus services built using Python and the re (regular expression) module. It responds to user queries like booking tickets, cancellation, refund, payment methods, and more. It identifies intent using keyword matching and replies with a suitable response.

✅ Features:
Handles common RedBus queries (booking, cancellation, refunds, etc.)

Case-insensitive matching

Uses re.search() for flexible pattern matching

Clean fallback message for unmatched inputs

Supports exit commands like "bye" or "exit"
...
Dictionary: Stores regex patterns and corresponding responses.

Regex (re module): Performs pattern matching.

✅ Time & Space Complexity:
Time Complexity: O(N) per message, where N is the number of regex patterns.

Space Complexity: O(N), storing N patterns and responses.

✅ Applications:
Customer service for travel booking websites

Rule-based helpdesk bots

Online ticketing platforms

✅ Viva Questions & Answers:
Q1: What type of chatbot is this?
A1: It's a rule-based chatbot using regular expressions.

Q2: What module is used for pattern matching?
A2: Python's built-in re module.

Q3: How does the chatbot understand user queries?
A3: It uses regex to search for keywords in the input text.

Q4: How can the chatbot be improved?
A4: By integrating NLP or machine learning for better understanding and context tracking.

Q5: What happens if no pattern matches the input?
A5: It returns a default fallback message asking the user to rephrase.'''